...
const selectData = ({limit,offset,sort,sortby,search}) => {
  ...
  const selectDataUser = ({limit,offset,sort,sortby,search,users_id}) => {
    ...
    return Pool.query(
      ...
      //(products.name) ILIKE ('%${search}%') ORDER BY products.${sortby} ${sort} LIMIT ${limit} OFFSET ${offset}
    );
  };
...
const insertData = (data) => {
    ...
    console.log(data)
    return Pool.query(`INSERT INTO products(name,stock,price,categorys_id,photo,users_id) 
    VALUES ('${name}',${stock},${price},${categorys_id},'${photo}','${users_id}')`)
}
